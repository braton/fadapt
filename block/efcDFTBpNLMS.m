function [w,y] = efcDFTBpNLMS( u,d,m,b,B,R,N )
% efcDFTBpNLMS - Constrained DFT Block Power Normalized Least Mean Square
%                with error estimate by efficient convolution in fullband
%                
% [w,y] = [w,y] = efcDFTBpNLMS( u,d,m,b,B,R,N )
%
% u         - Input signal;
% d         - desired signal;
% m         - step size;
...
for n=1:ceil(uLen/R)
    ...
    y(min(n*R,uLen):-1:(n-1)*R+1) = yR(max(n*R-uLen+1,1):R);
